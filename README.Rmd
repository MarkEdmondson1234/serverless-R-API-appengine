---
title: "Creating a serverless R API on Google Cloud Platform"
...
0. Install [plumber](https://www.rplumber.io/), [containerit](https://github.com/o2r-project/containerit), and the python [gcloud SDK](https://cloud.google.com/sdk/downloads)
...
4. Configure App Engine's `app.yaml`
...
5. Upload to a USA based App Engine via `gcloud app deploy --project your-project`
6. Your R code is now deployed as a Plumber app https://your-project.appspot.com/
...
6. Plumber generates a swagger.json file that is available at `https://your-project.appspot.com/swagger.json` or if you run it locally via `http://127.0.0.1:8080/swagger.json`
7. Run the function below to generate the openapi.yaml for Google cloud endpoints
...
8. Deploy the openapi.yaml in terminal via `gcloud service-management deploy openapi.yaml --project your-project`
9. Run `gcloud service-management configs list --service=your-project.appspot.com` to see the service management name and config you just uploaded
10. Add these lines to the `app.yaml` of the app engine to include the info you got from the listing:
